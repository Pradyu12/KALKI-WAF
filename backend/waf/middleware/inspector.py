...
_BYPASS_PATHS = frozenset({
    "/", "/dashboard", "/earth.jpg", "/kalki_waf_logo.png",
...
def log_incident_to_db(event_data: dict):
    ...
    args = (
        event_data["incident_id"], ts, event_data["source_ip"],
        event_data["user_agent"], event_data["target_uri"],
...
async def inspect_and_proxy_traffic(request: Request, call_next):
    ...
    if await geoip.check_country_block(client_ip):
        ...
        bg_tasks.add_task(
            log_incident_to_db,
            {
                "incident_id": incident_id, "timestamp": datetime.now(UTC),
                "source_ip": client_ip, "user_agent": user_agent,
                "target_uri": target_uri,
                "malicious_payload": f"GEO_BLOCKED:{blocked_country}",
                "threat_category": "GeoBlock", "mitigation_action": "Blocked",
            },
        )
    ...
    if not await rate_limiter.check_rate_limit(client_ip):
        ...
        bg_tasks.add_task(
            log_incident_to_db,
            {
                "incident_id": incident_id, "timestamp": datetime.now(UTC),
                "source_ip": client_ip, "user_agent": user_agent,
                "target_uri": target_uri, "malicious_payload": "RATE_LIMIT_EXCEEDED",
                "threat_category": "Anomalous", "mitigation_action": "Blocked",
            },
        )
                ...
                if not check_graphql_depth(json_body["query"]):
                    ...
                    bg_tasks.add_task(
                        log_incident_to_db,
                        {
                            ...
                            "threat_category": "GraphQL", "mitigation_action": "Blocked",
                        },
                    )
        ...
        if content_length:
            ...
            if cl > MAX_BODY_BYTES:
                ...
                return JSONResponse(
                    ...
                    status_code=413, background=bg_tasks,
                )
    ...
    if detected_threat:
        ...
        event_log = {
            ...
            "threat_category": detected_threat, "mitigation_action": action,
        }

        incident_payload = {
            "incident_id": incident_id, "source_ip": client_ip,
            "threat_category": detected_threat, "action": action,
