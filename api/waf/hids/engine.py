...
_AUTH_LOG_PATTERNS: list[tuple[str, str, str, str]] = [
    ...
    (r"DROP.*SRC=(\S+).*DPT=(\d+)", "fw_drop", "medium", "Firewall packet drop"),
...
def detect_bruteforce(source_ip: str = "unknown") -> dict[str, Any] | None:
    ...
    _recent_failures[source_ip] = [t for t in _recent_failures[source_ip] if now - t < _BRUTEFORCE_WINDOW]
...
def get_hids_stats() -> dict[str, Any]:
    ...
    by_severity = query_db("SELECT severity, COUNT(*) as cnt FROM hids_alerts GROUP BY severity ORDER BY cnt DESC")
    by_type = query_db("SELECT log_type, COUNT(*) as cnt FROM hids_alerts GROUP BY log_type ORDER BY cnt DESC")
    return {
