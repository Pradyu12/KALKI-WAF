...
SIEM_RULE_DEFINITIONS = [
    ...
    {
        "rule_id": "siem-anomaly-001",
        "rule_name": "Request Anomaly Score",
        "severity": "low",
        "description": "Flags requests with high entropy headers, rare user-agents, or unusual timing",
        "query": "SELECT COUNT(*) as cnt FROM siem_alerts WHERE rule_id = 'siem-anomaly-001' AND timestamp > datetime('now', '-5 minutes')",
        "threshold": 10,
        "cooldown": 60,
    },
]
...
def get_alert_stats() -> dict[str, Any]:
    ...
    by_severity = query_db("SELECT severity, COUNT(*) as cnt FROM siem_alerts GROUP BY severity ORDER BY cnt DESC")
    return {
