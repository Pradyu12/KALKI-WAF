...
from waf import state as _waf_state
...
def execute_playbook(
    playbook_id: str, target: str, rule_id: str | None = None, triggered_by: str = "admin"
) -> dict[str, Any]:
    ...
    try:
        if playbook_id == "block_ip_iptables":
            ...
            result = "success" if r.returncode == 0 else f"failed: {r.stderr.strip()}"
            ...
            _waf_state.GLOBAL_POSTURE = "Under Attack"
            result = "success"
        ...
        if result == "success" or result == "triggered":
            ...
            ingest_log(
                "active_response",
                "playbook_executed",
                f"Playbook {playbook_id} executed on {target}: {result}",
                playbook["severity"],
            )
...
def get_response_stats() -> dict[str, Any]:
    ...
    by_status = query_db("SELECT status, COUNT(*) as cnt FROM active_response_log GROUP BY status")
    return {
