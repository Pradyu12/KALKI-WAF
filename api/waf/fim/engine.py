...
def check_integrity(path: str) -> dict[str, Any] | None:
    baseline = query_db("SELECT * FROM fim_baseline WHERE file_path = ?", (path,), one=True)
    if not baseline:
        record_baseline(path)
        return None
...
def get_fim_events(limit: int = 50, offset: int = 0, change_type: str | None = None) -> list[dict[str, Any]]:
    if change_type:
        rows = query_db(
...
def get_fim_stats() -> dict[str, Any]:
    ...
    by_type = query_db("SELECT change_type, COUNT(*) as cnt FROM fim_events GROUP BY change_type")
