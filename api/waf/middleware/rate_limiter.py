...
async def check_rate_limit(client_ip: str) -> bool:
    ...
    with _rate_lock:
        ...
        if (
            len(state.request_history) > 10000
            or len(state.request_history) > 100
            and client_ip
            and hash(client_ip) % 100 == 0
        ):
...
def _cleanup_stale_ips(now: float | None = None):
    ...
    with _rate_lock:
        stale = [
            ip for ip, times in state.request_history.items() if not times or now - max(times) > RATE_LIMIT_WINDOW * 2
        ]
        for ip in stale:
